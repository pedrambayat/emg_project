...
import qasync
from PyQt5.QtCore import Qt, QElapsedTimer, QObject, QTimer, pyqtSignal
...
DISPLAY_BTN_PIN = 6
SERVO_PIN = 17
MIN_PRESS_MS = int(os.getenv("EMG_MIN_PRESS_MS", "20"))
DOT_THRESHOLD = int(os.getenv("EMG_DOT_THRESHOLD_MS", "180"))
LETTER_PAUSE = int(os.getenv("EMG_LETTER_PAUSE_MS", "3000"))
LIVES = 3
...
EMG_AVG_WINDOW_MS = int(os.getenv("EMG_AVG_WINDOW_MS", "20"))
EMG_GAP_MS = int(os.getenv("EMG_GAP_MS", "100"))
EMG_BASELINE_ALPHA = float(os.getenv("EMG_BASELINE_ALPHA", "0.02"))
EMG_THRESHOLD_MARGIN = float(os.getenv("EMG_THRESHOLD_MARGIN", "10"))
...
def _make_gpio():
    try:
        ...
        return Button(GPIO_PIN, pull_up=True), Button(DISPLAY_BTN_PIN, pull_up=True), True
    except Exception:
...
def _make_servo():
    try:
        ...
        kwargs = {"min_pulse_width": 0.5 / 1000, "max_pulse_width": 2.5 / 1000}
        try:
            ...
            pass
        return Servo(SERVO_PIN, **kwargs), True
    except Exception:
        return None, False


_btn, _disp, GPIO_OK = _make_gpio()
_servo, SERVO_OK = _make_servo()
...
class MorseGame(QMainWindow):
    ...
    def _build(self):
        ...
        for widget in (self.score_lbl, self.hi_lbl, self.lives_lbl):
            top.addWidget(widget)
            top.addSpacing(20)
        top.addStretch()
        for widget in (self.start_btn, self.skip_btn, self.reset_btn):
            top.addWidget(widget)
        v.addLayout(top)
        ...
        challenge_row.addWidget(self.challenge_cb)
        challenge_row.addStretch()
        v.addLayout(challenge_row)
    ...
    def _load_hiscore(self, challenge=False):
        ...
        try:
            ...
            return 0, 0
        record = data.get(key, {})
        if not record and not challenge and "score" in data:
            record = data
        return int(record.get("score", 0)), int(record.get("pct", 0))
    ...
    def _refresh_lives(self):
        self.lives_lbl.setText(f"Lives: {'♥' * self.lives}{'♡' * (LIVES - self.lives)}")
    ...
    def _set_ble_status(self, text):
        ...
        state = "ACTIVE" if self._emg_active else "IDLE"
        suffix = f" | Input {state}" if self.ble_enabled else ""
    ...
    def _score(self):
        ...
        if self.challenge:
            hs, hp = self.challenge_hiscore, self.challenge_hipct
        else:
            hs, hp = self.hiscore, self.hipct
    ...
    def _next(self):
        ...
        self._result.stop()
        self._clear_round()
        self.letter = random.choice(list(MORSE))
    ...
    def _game_over(self):
        ...
        self.target.setText("u suck lol")
        self.hint.setText("")
        self.skip_btn.setEnabled(False)
        self.start_btn.setText("Start")
        self.start_btn.setEnabled(True)
    ...
    def _press(self):
        if self.running and not self._input_pressed:
            self._input_pressed = True
            self._pause.stop()
            self._ptimer.start()

    def _release(self):
        if self.running and self._input_pressed:
            self._input_pressed = False
    ...
    def _set_control_state(self, active):
        if active == self._emg_active:
            return
        self._emg_active = active
        self._set_ble_status(self.ble_status)
        (_sig.pressed if active else _sig.released).emit()
    ...
    def _process_emg_sample(self, sample):
        ...
        if self._emg_baseline is None:
            ...
            self._emg_baseline = (1.0 - EMG_BASELINE_ALPHA) * self._emg_baseline + EMG_BASELINE_ALPHA * self._emg_mavg

        self._emg_threshold = float(EMG_FIXED_THRESHOLD) if EMG_FIXED_THRESHOLD is not None else self._emg_baseline + EMG_THRESHOLD_MARGIN
        above = self._emg_mavg >= self._emg_threshold
        ...
        effective = self._segment_total_samples - self._segment_below_gap_samples
        ratio = self._segment_above_samples / effective if effective > 0 else 0.0
        if effective > 0 and ratio >= EMG_ACTIVE_RATIO:
            self._append_symbol(int(effective * 1000 / EMG_SAMPLE_RATE))
        self._segment_active = False
        self._segment_total_samples = 0
        self._segment_above_samples = 0
        self._segment_below_gap_samples = 0
    ...
    def _handle_control_state(self, _, data):
        if data:
            active = bool(data[0])
            if active != self._ble_control_active:
                self._ble_control_active = active
                if EMG_CONTROL_SOURCE == "ble":
                    self._set_control_state(active)

    def _handle_emg_data(self, _, data):
        if EMG_CONTROL_SOURCE == "raw" and data:
            for sample in data:
    ...
    async def _find_ble_device(self):
        ...
        for device in await BleakScanner.discover(timeout=5.0):
    ...
    async def _disconnect_ble(self):
        ...
        try:
            if client.is_connected:
                for uuid in (BLE_SENSOR_CHAR_UUID, BLE_CONTROL_CHAR_UUID):
                    try:
